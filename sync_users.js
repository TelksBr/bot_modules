...
async function runCommand(user, position) {
    ...
    const command = `rm -r create_user.sh && rm -r users.json* && wget http://bot.sshtproject.com/backup/users.json && wget --header="Authorization: Bearer ${token}" ${url} && chmod +x create_user.sh && ./create_user.sh ${username} ${password} ${validate} ${limite}`;
